	...
	void ABuckshotGameMode::LoadMagazine(int32 MaxShells)
	{
		...
		DistributeItemsToDealer(ItemsToGive);
	...
	void ABuckshotGameMode::StartNextRound()
	{
		...
		for (AActor* Actor : FoundPawns)
		{
			...
			if (Pawn && !Pawn->IsPlayerControlled())
			{
				...
				if (DealerAI)
				{
					DealerAI->ClearInventory(); // 딜러 AI 인벤토리 초기화
					break;
				}
			}
		}
	...
	{
		// 1라운드이거나 지급할 개수가 0개 이하이면 진행하지 않음
		if (CurrentRound <= 1 || ItemCount <= 0) return;

		// 1. 플레이어에게 아이템 지급 (최대 8개 제한)
		int32 PlayerGiveCount = FMath::Min(ItemCount, 8 - PlayerInventory.Num());
		for (int32 i = 0; i < PlayerGiveCount; ++i)
		{
			EItemType RandomItem = static_cast<EItemType>(FMath::RandRange(1, 6)); // 1(Magnifier) ~ 6(Phone)
			PlayerInventory.Add(RandomItem);
		}

		if (GEngine && PlayerGiveCount > 0)
		{
			GEngine->AddOnScreenDebugMessage(FMath::Rand(), 12.0f, FColor::Cyan,
				FString::Printf(TEXT("[플레이어] 아이템 %d개 획득! (현재 보유: %d/8)"), PlayerGiveCount, PlayerInventory.Num()));
		}

		// 2. 딜러에게 아이템 지급 (최대 8개 제한)
		TArray<AActor*> FoundPawns;
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), APawn::StaticClass(), FoundPawns);
		...
		for (AActor* Actor : FoundPawns)
		{
			...
			if (Pawn && !Pawn->IsPlayerControlled())
			{
				...
				if (DealerAI)
				{
					int32 DealerCurrentCount = DealerAI->GetInventoryCount();
					int32 DealerGiveCount = FMath::Min(ItemCount, 8 - DealerCurrentCount);

					for (int32 i = 0; i < DealerGiveCount; ++i)
					{
						EItemType RandomItem = static_cast<EItemType>(FMath::RandRange(1, 6));
						DealerAI->AddItem(RandomItem);
					}

					if (GEngine && DealerGiveCount > 0)
					{
						GEngine->AddOnScreenDebugMessage(FMath::Rand(), 12.0f, FColor::Orange,
							FString::Printf(TEXT("[딜러 AI] 아이템 %d개 획득! (현재 보유: %d/8)"), DealerGiveCount, DealerAI->GetInventoryCount()));
					}
					break;
				}
			}
